...
# internal crates are located in `components/`
CRATES="
	addr2line@0.21.0
	adler@1.0.2
	ahash@0.7.7
...
SRC_URI="
	${CARGO_CRATE_URIS}
	https://github.com/getzola/${PN}/archive/v${PV}.tar.gz -> ${P}.tar.gz
"
...
src_install() {
	...
	local zola="${ED}/usr/bin/zola"

	edo "${zola}" completion bash | newbashcomp - "${PN}.bash"
	edo "${zola}" completion fish | newfishcomp - "${PN}.fish"
	edo "${zola}" completion zsh  | newzshcomp  - "_${PN}"
}
