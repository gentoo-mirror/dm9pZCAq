...
IUSE="aqua dbus +egl examples gamepad +geolocation gles2-only gnome-keyring +gstreamer gtk-doc +introspection +jpeg2k +jumbo-build libnotify +opengl seccomp spell systemd wayland +X"
...
src_prepare() {
	...
	(
		...
		__src='^\(\s\+\)assert\s*\(.*\)==\s*\(memoryBits\)$'
		__dst='\1assert math.isclose(\2, \3)'

		# fixing python error when comparing float and int
		sed -i "${S}/Source/JavaScriptCore/wasm/generateWasm.py" \
			-e "s/${__src}/${__dst}/" \
		&& einfo 'patched: generateWasm.py'

		# fixing missing header
		sed -i "${S}/Source/JavaScriptCore/jsc.cpp" \
			-e 'x;/./{x;b};x;/<wtf/h;//i#include <clocale>' \
		&& einfo 'patched: jsc.cpp'
